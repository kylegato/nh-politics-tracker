...
export function createMetricsCollector() {
  ...
  // Instead of using setInterval (which isn't allowed in global scope),
  // we'll check and clean up during regular request processing
  ...
  return {
    ...
    recordRequest() {
      ...
      // Check if we need to clean up request durations array
      // Do this every minute of processing instead of using setInterval
      const now = Date.now();
      if (now - metrics.lastCleanupTime > 60000) {
        metrics.requestDurations = metrics.requestDurations.slice(-100);
        metrics.lastCleanupTime = now;
      }
    },
 ...
 */
export function createHealthCheckHandler(metricsCollector) {
